...
def get_kl_div(
    ...
    if torch.isnan(pos_pi_logits).any() or torch.isnan(pos_ref_logits).any():
        print("🚨 NaN detected in KL divergence logits! Exiting...")
        exit()
...
class BasicTrainer(object):
    ...
    def concatenated_forward(
        ...
        if torch.isnan(all_logits).any():
            print("🚨 NaN detected in model logits BEFORE softmax! Exiting...")
            print("Logits before softmax:", all_logits)
            exit()
    ...
    def get_batch_metrics(
        ...
        if torch.isnan(losses).any():
            print("🚨 NaN detected in loss before backward pass! Exiting...")
            exit()
    ...
    def train(self, batch):
        ...
        for name, param in self.policy.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"🚨 NaN detected in gradients of {name}! Exiting...")
                exit()
