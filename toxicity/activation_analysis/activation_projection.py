...
model_name = "mistralai/Mistral-7B-v0.1" # "google/gemma-2-2b" # "google/gemma-2-2b" # "mistralai/Mistral-7B-v0.1" # "meta-llama/Llama-3.1-8B" # "gpt2-medium" # "meta-llama/Llama-3.1-8B" # "google/gemma-2-2b", # "gpt2-medium", # "mistralai/Mistral-7B-v0.1",
dpo_model_name = "mistral_dpo_0.05_final.pt" # "gemma2_2b_dpo_0.05_final.pt" # "llama3_dpo_0.1_attn_final.pt" # "mistral_dpo.pt" # "gpt2_dpo.pt" # "llama3_dpo_2.pt"
probe_name = "mistral_probe.pt" # "gemma2_2b_probe.pt" # "mistral_probe.pt" # "gpt2_probe.pt" # "llama3_probe.pt"
model_short_name = "mistral" # "gemma2" "mistral" #"gpt2"
BATCH_SIZE = 512
...
#     neuron_act_sums = defaultdict(lambda: torch.zeros(model.config.intermediate_size, dtype=torch.float16, device=device)) # m_i - scalar
...
#             value_vectors = value_vectors.to(toxic_vector.dtype)  # (d_mlp, d)
#             value_norms = torch.norm(value_vectors, dim=1)  # (d_mlp,)
#             v = torch.matmul(value_vectors.to(toxic_vector.dtype), toxic_vector) / (value_norms * toxic_norm)  # (d_mlp,)
#             print(f"Layer {layer_idx} value vector projection shape (d_mlp): {v.shape}")
            
#             # Get activations for last 20 tokens only
#             neuron_acts_gen = neuron_acts[:, -20:, :]  # (B, 20, d_mlp)
#             print(f"Extracting last 20 tokens (B, 20, d_mlp): {neuron_acts_gen.shape}")

#             # Sum activations over B*20 → shape: (d_mlp,)
#             neuron_act_sum = neuron_acts_gen.sum(dim=(0, 1))  # (d_mlp,)
#             neuron_proj_sum = neuron_act_sum * v # (d_mlp,) - Elementwise-multiplication
#             neuron_count = neuron_acts_gen.shape[0] * neuron_acts_gen.shape[1]  # B × 20

#             # Accumulate across batches
#             neuron_act_sums[layer_idx] += neuron_act_sum
#             neuron_proj_sums[layer_idx] += neuron_proj_sum
#             neuron_counts[layer_idx] += neuron_count
            
#             print(f"Updated neuron activation sums for layer {layer_idx} with shape (d_mlp): {neuron_act_sums[layer_idx].shape}") # (d_mlp)
#             print(f"Updated neuron projection sums for layer {layer_idx} with shape (d_mlp): {neuron_proj_sums[layer_idx].shape}") # (d_mlp)
#             print(f"Total neuron counts: {neuron_counts[layer_idx]}") # B*20
